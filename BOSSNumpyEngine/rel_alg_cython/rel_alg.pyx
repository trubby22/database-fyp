...
ctypedef dict[str, list[cnp.int_t[:]]] table_spans
ctypedef dict[str, cnp.int_t[:]] table_column
...
def project(table_spans table, list[str] col_names):
    return {col_name : table[col_name] for col_name in col_names}

# works on materialised columns
def select(table_spans table_in, list[str] key_col_names, list[str] boolean_ops, list vals):
    cdef table_column table = materialise_spans_into_columns(table_in)

    cdef list[str] col_names = list(table.keys())
...
def equi_join(table_spans table_1_in, table_spans table_2_in, list[str] key_col_names_1, list[str] key_col_names_2):
    cdef table_column table_1 = materialise_spans_into_columns(table_1_in)
    cdef table_column table_2 = materialise_spans_into_columns(table_2_in)

    cdef list[str] col_names_1 = list(table_1.keys())
    cdef list[str] col_names_2 = list(table_2.keys())
    cdef list[str] keys_1 = [table_1[col_name] for col_name in key_col_names_1]
    cdef list[str] keys_2 = [table_2[col_name] for col_name in key_col_names_2]
...
# accepts and returns tables /w materialised columns
def aggregate(table_spans table_in, list[str] key_col_names, str reduction_func, str reduction_col_name):
    cdef table_column table = materialise_spans_into_columns(table_in)

    cdef list[str] col_names = list(table.keys())
    cdef list[str] key_cols = [table[col_name] for col_name in key_col_names]
...
cdef int np_prod(cnp.int_t[:] x):
    return np.prod(x)

cdef int np_count(cnp.int_t[:] x):
    return x.size

cdef int np_avg(cnp.int_t[:] x):
    return np.mean(x)

cdef int np_max(cnp.int_t[:] x):
    return np.max(x)

cdef int np_min(cnp.int_t[:] x):
    return np.min(x)
